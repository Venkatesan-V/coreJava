...
public class ZDishRunner {

//	Requirement: Pick Veg items and sort LowToHigh
...
//		Sort L2H
	...
	public static void getVegItems () {
		List<Dish> vegList =  menu.stream()			//menu(Collection) is converted into stream
		.filter(Dish::isVeg)				//filter only NV items
...
//		.map(dish->dish.getDishName())					//fetch sorted items
	...
	public static void getVegItemsRev() {
		List<String> javaEightRev =  menu.stream()
		.filter(dish-> dish.isVeg()==true)
